  ...
  <div class="absolute inset-0 bg-black/80"></div>
    <div class="relative z-10">
      <div class="flex container mx-auto px-4 py-12 lg:py-20 gap-10 items-center">
        <img src="{{ $albumData['cover'] }}" alt="{{ $albumData['nombre'] }}"
             class="w-40 h-40 object-cover rounded-lg shadow">
        <div>
            <h1 class="text-3xl font-bold">{{ $albumData['nombre'] }}</h1>
            <p class="text-neutral-400">{{ $albumData['anio'] }} • {{ $albumData['pistas'] }} canciones</p>
        </div>
      </div>
      ...
      <div class="card p-6 mx-auto">
        ...
        <ul class="divide-y divide-white/10">
            @foreach ($albumData['canciones'] as $c)
